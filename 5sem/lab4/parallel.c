...
int main(int argc, char *argv[]) {
    ...
    double *local_newgrid =
        xcalloc((ny + 2) * (nx + 2), sizeof(*local_newgrid));
    ...
    ttotal += MPI_Wtime();
    if (rank == 0)
        printf("# Heat 2D (mpi): grid: rows %d, cols %d, procs %d (px %d, py "
               "%d)\n",
    ...
    if (rank == 0) {
        ...
        printf("# procs %d : grid %d %d : niters %d : total time %.6f : mpi "
               "time %.6f : allred %.6f : halo %.6f : speedup %.6f\n",
               commsize, rows, cols, niters, prof[0], prof[1] + prof[2],
               prof[1], prof[2], 526.82344 / prof[0]);
    } else {
